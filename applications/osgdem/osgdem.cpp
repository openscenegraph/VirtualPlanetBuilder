...
void catchSignal(int data)
{
    // make sure all changes to files appear on disk
    sync();
    
    // print out the signal.
    psignal(data,"Signal caught");
}

void exitOccured(int status, void* arg)
{
    printf("exitOccured\n");
}
...
int main(int argc, char** argv)
{
    ...
    signal(SIGTSTP, catchSignal);
    signal(SIGXFSZ, catchSignal);

    on_exit (exitOccured, 0);
#endif
        ...
        if (node.valid())
        {
            ...
            if (loaded_terrain) 
            {
                ...
                if (loaded_db)
                {
                    ...
                    osg::notify(osg::INFO)<<"Reassigning buildOptions"<<std::endl;
                }
                else
                {
                    loaded_terrain->setTerrainTechnique(databaseBuilder);
                    osg::notify(osg::INFO)<<"Reusing buildOptions"<<std::endl;
                }
    ...
    if (arguments.read("--daemon"))
    {
        // run osgdem in the background
