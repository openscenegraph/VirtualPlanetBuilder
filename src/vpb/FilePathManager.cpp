/* -*-c++-*- VirtualPlanetBuilder - Copyright (C) 1998-2008 Robert Osfield 
 *
...
osgDB::FileType FilePathManager::getFileType(const std::string& filename)
{
    ...
    if (itr != _filePathTypeMap.end()) return itr->second;
    
    osgDB::FileType type = osgDB::fileType(filename);
    if (type==osgDB::FILE_NOT_FOUND) return osgDB::FILE_NOT_FOUND;
...
bool FilePathManager::checkWritePermissionAndEnsurePathAvailability(const std::string& filename)
{
    ...
    if (itr != _filePathWritePermissionMap.end()) return itr->second;
